<form id="cari_absen" class="form-horizontal" method="get" action="{!! route('be.transaksi',[$data['page'],$data['subpage']]) !!}">
     @if(!empty($request->get('menu')) or @$data['page']=='non_ajuan' or @$data['page']=='ajuan' or @$data['page']=='direksi' or @$data['page']=='voucher')
   
     <div class="card">
        @if(@$data['page']=='non_ajuan' and $request->get('preview')!='preview' or @$data['page']=='direksi' or @$data['page']=='voucher')
        <div class="card-header">
            ...
            @if(@$request->get('preview')!='Gaji' )
	            ...
	            <a href="{!! route('be.tambah_generate_'.$data['subpage']) !!}" target="_blank" class="btn btn-sm btn-primary" title='Tambah Generate Gaji' data-toggle='tooltip'><span class='fa fa-plus'></span> Tambah Generate <?=ucwords($data['subpage'])?> </a>
	              @endif
	            @if(@$data['page']=='direksi' or @$data['page']=='non_ajuan'  or @$data['page']=='voucher' )
	            <a href="{!! route('be.transaksi', [$data['page'],$data['subpage'], 'preview=Gaji']) !!}" target="_blank" class="btn btn-sm btn-primary" title='Preview' data-toggle='tooltip'><span class='fa fa-plus'></span><?php if($data['page']=='direksi') echo 'Preview'; else echo 'Ajuan '.ucwords($data['subpage']);?> </a>
	              @endif
         @elseif(@$data['page']=='non_ajuan' and $request->prl_generate and $data['subpage']=='gaji')
            <a href="{!! route('be.proses_generate', $request->prl_generate) !!}" target="_blank" class="btn btn-sm btn-primary" title='Regenerate' data-toggle='tooltip'><span class='fa fa-reset'></span>
            	Regenerate
             </a> <a href="{!! route('be.regenerate_pajak', $request->prl_generate) !!}" target="_blank" class="btn btn-sm btn-primary" title='Regenerate Pajak' data-toggle='tooltip'><span class='fa fa-reset'></span>
            	Regenerate Pajak
             </a>
         @elseif(@$data['page']=='non_ajuan' and $request->prl_generate and $data['subpage']=='thr')
            <a href="{!! route('be.proses_generate_thr', $request->prl_generate) !!}" target="_blank" class="btn btn-sm btn-primary" title='Regenerate' data-toggle='tooltip'><span class='fa fa-reset'></span>
            	Regenerate
             </a> 
         @endif
        ...
        <div class="card-body">
            ...
            <input type="hidden" name="preview" value="Gaji"/>
                <div class="col-lg-8 row">
                    ...
                    <div class="form-group col-md-6">
                        ...
                        <select class="form-control select2" name="prl_generate"  id="prl_generate" style="width: 100%;" required onchange="getpekananbulananselect(this)">
                            ...
                            foreach ($periode as $periode) {
                               $selected = '';
                                ...
                                if($data['subpage']=='gaji'){
                                	if($periode->periode_gajian ==2){
                                		$text="";
                                	}else{
                                		$text = '| Absen:' . $periode->tgl_awal . ' - ' . $periode->tgl_akhir . ' | Lembur:' . $periode->tgl_awal_lembur . ' - ' . $periode->tgl_akhir_lembur;
                                	}
                                    echo '<option '.$selected.' value="' . $periode->prl_generate_id . '">' . $periode->tipe . ' - Periode: ' . $periode->tahun_gener . ' Bulan: ' . $periode->bulan_gener .$text . '</option>';
                                }
                    ...
                    <div class="form-group col-md-6" >
                        ...
                        <select class="form-control " name="periode_gajian" id="periode_gajian" style="width: 100%;" required >
                            <option value="">Pilih Periode Gajian</option>
                            <option value="1" <?=$request->periode_gajian==1?'selected':''?>>Bulanan</option>
                            <option value="2" <?=$request->periode_gajian==2?'selected':''?>>Pekanan</option>
            ...
            <div class="form-group row">
                ...
                <div class="col-md-7">
                    ...
                    <button type="submit" name="Cari" class="btn btn-primary" value="Absen"><span class="fa fa-search"></span> Detail Absen</button>
                </div>
                <div class="col-md-5">
                  ...
                  <button type="button" onclick="undo_appr()" name="Cari" class="btn btn-primary pull-right" value="Approval"><span class="fa fa-file-excel"></span> Undo Appr Data</button>
                    ...
                    <button type="button" onclick="undo_appr_voucher()" name="Cari" class="btn btn-primary pull-right" value="Approval"><span class="fa fa-file-excel"></span> Undo Appr Vo	ucher</button>
                ...
                <div class="col-md-9">
                    ...
                    <button type="submit" name="Cari" class="btn btn-primary" value="Ajuan"><span class="fa fa-search"></span> Ajuan</button>
                     ...
                     <button type="submit" name="Cari" class="btn btn-primary" value="RekapExcel"><span class="fa fa-file-excel"></span> Excel Detail Gaji</button>
                      <button type="submit" name="Cari" class="btn btn-primary" value="EditKaryawan"><span class="fa fa-search"></span> Edit Data Karyawan</button>
                      <button type="submit" name="Cari" class="btn btn-primary" value="Absen"><span class="fa fa-search"></span> Detail Absen</button>
                </div>
   	...
   	@endif  
        ...
        <script type="text/javascript">
			...
			alert()
			getpekananbulananselect();
    			function getpekananbulananselect() {
            	
                txt = $("#prl_generate option:selected").text();
               //alert(txt.substring(0, 18))
                if(txt.substring(0, 19)=="Pekanan dan Bulanan"){
                	$('#periode_gaji').show();
                	$('#periode_gajian').val('');
                	
                }else{         
	                if(txt.substring(0, 7)=='Pekanan')       	
	                	$('#periode_gajian').val(0);
	                else
	                	$('#periode_gajian').val(1);
                	$('#periode_gaji').hide()
                }
            ...
            function submit_approve() {
                ...
                if (entitas) {
                    $('#pesan').html('')
                    $('#submit_appr').val('Undo Approve');
                    $('#Cari_submit').val('Cari');
                    
                    swal({
                        title: 'Undo Approval Data Gaji Entitas ' + txt + ' ' + pajakonoff + '?',
                        text: "Submit Undo Approval Data!",
                        type: 'warning',
                        showCancelButton: true,
                        confirmButtonColor: '#3085d6',
                        cancelButtonColor: '#d33',
