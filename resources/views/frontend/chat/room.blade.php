...
<h4 class="card-title float-left mb-0 mt-2"><?= $chat_room->tujuan==1?'Absensi': 'Gajian';?></b> - <?= $chat_room->topik;?> </h4>
...
<section class="msger">
  <header class="msger-header">
    <div class="msger-header-title">
      <i class="fas fa-comment-alt"></i> <?php  if($chat_room->tujuan==1) echo 'Absensi - Finger tidak terbaca'; else 
                                  ...
                                  if($chat_room->tujuan==9) echo 'Lainnya';  ?></b> - <?= $chat_room->topik;?> 
                                   @if(!empty($chat_room->file))
                                   <a href="{!! asset('dist/img/file/'.$chat_room->file) !!}" target="_blank" title="Download"><span class="fa fa-download"></span></a>
                                   @endif
      <br><span class="text-muted">Deskripsi: </span><?= $chat_room->deskripsi;?> 
    </div>
   
  </header>

  <main class="msger-chat" id="contentMassage">
   
  </main>

  <form class="msger-inputarea">
    <input type="text" class="msger-input" placeholder="Enter your message...">
    <button type="button" class="msger-send-btn" onclick="send()">Send</button>
  </form>
</section>
...
<script>
	function content(){
		...
		$.ajax({
				type: 'get',
				//data: {'id': id},
				url: '<?=route('fe.content_chat',$id_chat);?>',
				dataType: 'html',
	...
	function send(){
		...
		$.ajax({
				type: 'get',
				data: {'pesan': pesan},
				url: '<?=route('fe.send_chat',$id_chat);?>',
				dataType: 'html',
				success: function(data){
					//$('.msger-chat').html(data);
