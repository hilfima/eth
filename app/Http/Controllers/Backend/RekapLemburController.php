...
left join p_karyawan_pekerjaan on p_karyawan_pekerjaan.p_karyawan_id=p_karyawan.p_karyawan_id
		...
		$sqllembur = "Select * from t_permit where tgl_awal>='$tgl_awal' and tgl_awal<='$tgl_akhir 23:00' and m_jenis_ijin_id = 22 and status_appr_1 = 1 ORDER BY p_karyawan_id asc";
		...
		$sql = "SELECT c.p_karyawan_id,c.nama,c.nik,m_lokasi.kode as nmlokasi,m_jabatan.nama as nmjabatan 
		FROM p_karyawan c 
		...
		WHERE 1=1 $where $whereLokasi $appendwhere order by c.nama";
		...
		if($request->get('Cari')=="RekapExcel"){
			...
			return view('backend.rekap_lembur.view',compact('tgl_awal','tgl_akhir','bulan','tahun','periode','periode_absen','periode_gajian','help','tgl_awal','tgl_akhir','list_karyawan','rekap','user','hariLibur'));
		}
	}
