...
where users.id=$iduser";
        ...
        if($user[0]->role==3	  ){ 
		 $id_lokasi = $user[0]->m_lokasi_id;
			$whereLokasi = "AND p_karyawan_pekerjaan.m_lokasi_id = $id_lokasi";			
...
LEFT JOIN m_departemen on m_departemen.m_departemen_id=p_karyawan_pekerjaan.m_departemen_id
    ...
    }public function simpan_cek_absen_hr(Request $request,$id)
    {
		...
		try{
            ...
            return redirect()->route('be.cek_cari_absen_hr')->with('success',' Absen Karyawan Berhasil di input!');
        }
        catch(\Exeception $e){
