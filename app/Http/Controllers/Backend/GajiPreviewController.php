...
class GajiPreviewController extends Controller
{
    ...
    {
        $iduser = Auth::user()->id;
        $sqluser = "SELECT m_role.*,p_recruitment.foto,role,p_karyawan_pekerjaan,p_karyawan_pekerjaan.m_lokasi_id,user_entitas_access,m_role_id FROM users
        ...
        } else if ($request->get('submit_appr') == "Undo Approve") {
            GajiPreviewController::undo_appr_gaji($request);
        ...
        if ($request->get('prl_generate')) {
            ...
            $sql = "SELECT prl_generate.*,a.name as appr_nama 
            FROM prl_generate left join users a on prl_generate.appr_by = a.id 
            where prl_generate_id=$id_prl $where_periode";
            $generate = DB::connection()->select($sql);
            if ($subpage == 'gaji') {
                if ($generate[0]->periode_gajian == 1 or ($generate[0]->periode_gajian == 2 and $request->periode_gajian == 1)) {
                ...
                } else if ($generate[0]->periode_gajian == 0 or ($generate[0]->periode_gajian == 2 and $request->periode_gajian == 0)) {
                    ...
                    $data_row[] = array(
                        'Gaji Total Perbulan', 'gaji_total_per_bulan', 3, '',
                        array(
                            array(
                                "Kali", array(
                    ...
                    $data_row[] = array(
                        'Gaji Total Upah Harian', 'gaji_total_upah_harian', 3, 'pendapatan',
                        array(
                            array(
                                "Kali", array(
                                    array("field", 'ha'),
                    ...
                    $data_row[] = array('Lembur Proposional', 'lemburproposional', 2, 'pendapatan');
                    ...
                    $data_row[] = array(
                        'Total Pendapatan', 'jumlah_pendapatan_tunjangan', 4, 'total_pendapatan_tunjangan_karyawan',
                        array(
                            array(
                                "Tambah", array(
                                    array("field", 'gaji_total_upah_harian'),
                ...
                $data_row = array();
                $data_head = array();
                if ($request->periode_gajian == 1) {
                    $v = true;
                    $data_head[] = array('Pendapatan', 4, 'col');
                } else if (!($request->preview != 'preview' or $request->view != 'view')) {

                    $v = false;
                    $data_head[] = array('Pendapatan', 2, 'col');
                } else {
                    $v = true;

                    $data_head[] = array('Pendapatan', 4, 'col');
                }
                $data_head[] = array('THR', 2, 'col');
                $data_head[] = array('Potongan', 1, 'col');
                $data_head[] = array('Total', 1, 'col');
                ...
                $data_row[] = array(
                    'Perhitungan THR', 'perhitungan_thr', 4, 'total_thr_karyawan',
                    array(
                        array("Kali", array(
                            array("field", 'jumlah_pendapatan'),
                            array("field", 'total_bulan_gabung')
                        )),
                        array(
                            "Bagi", array(
                                array("num", '12'),
                ...
                $data_row[] = array('Zakat', 'jumlah_potongan', 2, 'potongan');
            ...
            if ($generate[0]->periode_gajian == 2) {

                $sqlperiode = "SELECT * FROM prl_gaji where prl_generate_id=$id_prl  and periode_gajian_in_gajian = " . $request->periode_gajian;
            } else {
            ...
            if ($subpage == 'gaji') {
                if ($generate[0]->periode_gajian == 2) {
                    if ($request->periode_gajian == 1)
                        $periode_absen = $generate[0]->periode_absen_bulanan_id;
                    else
                        $periode_absen = $generate[0]->periode_absen_pekanan_id;
                } else {
            ...
            if (($request->periode_gajian == 2 or $request->periode_gajian == 1) and !empty($request->periode_gajian)) {
                $p = $request->periode_gajian;
                if ($request->periode_gajian == 2) {
                    $p = 0;
                }
                $where_periode_gaji = "and k.periode_gajian = " . $p;
            }
            ...
            $wherebank = '';
            $where_directur = "";
            if ($request->get('pajakonoff') and $request->get('pajakonoff') != 'All')
                $wherepajak = "and k.on_off='" . $request->get('pajakonoff') . "'";
            if ($request->get('bank'))
                $wherebank = "and r.m_bank_id='" . $request->get('bank') . "'";
            if ($request->get('entitas') and $request->get('entitas') != 'All')
            ...
            $id_lokasi = Auth::user()->user_entitas;
            if ($id_lokasi and $id_lokasi != -1)
                $whereLokasirole = "AND k.lokasi_id in($id_lokasi)";
            else {
            ...
            if ($user[0]->m_role_id == 12 or ($page == 'direksi')) {
                ...
                $where_directur = " and  (
				(k.lokasi_id  in ($m_lokasi_hr_appr_on) and on_off='ON') 
					or (k.lokasi_id  in ($m_lokasi_hr_appr_off) and on_off='OFF')
					)";
            }
            $where_keuangan = "";
            ...
            if (($page == 'ajuan'))
                $where_keuangan = " and  (
				(k.lokasi_id  in ($m_lokasi_direktur_appr_on) and on_off='ON') 
					or (k.lokasi_id  in ($m_lokasi_direktur_appr_off) and on_off='OFF')
					)";


            $sql = "SELECT c.p_karyawan_id, " . ($generate[0]->periode_gajian ? $generate[0]->periode_gajian : -1) . " as gajian_type,c.nama as nama_lengkap,c.nik,m_lokasi.kode as nmlokasi,m_lokasi.nama as nmentitas,tgl_bergabung,
            f.m_pangkat_id,i.nama as nmpangkat ,
            f.nama as nmjabatan,g.tgl_awal,AGE(CURRENT_DATE, c.tgl_bergabung)::VARCHAR AS umur,f.job as jobweight , j.nama_grade as grade,k.no_rek as norek,k.lokasi_id as m_lokasi_id, k.on_off as pajak_onoff,r.nama_bank,keterangan_ajuan,prl_generate_karyawan_id,k.periode_gajian,case k.periode_gajian when 1 then 'Bulanan' else 'Pekanan' end as type_gaji, k.jabatan_id as m_jabatan_id,k.m_bank_id, k.periode_gajian as kperiode_gajian
            
			FROM p_karyawan c 
			JOIN prl_generate_karyawan k on c.p_karyawan_id=k.p_karyawan_id and k.status=1 and k.prl_generate_id = $id_prl $where_periode_gaji
            ...
            //(select count(*) from p_karyawan_koperasi where nama_koperasi='ASA' and active=1 and b.p_karyawan_id = p_karyawan_koperasi.p_karyawan_id) as is_koperasi_asa, (select count(*) from p_karyawan_koperasi where nama_koperasi='KKB' and active=1 and b.p_karyawan_id = p_karyawan_koperasi.p_karyawan_id) as is_koperasi_kkb,
            $sql = "select b.keterangan as keterangan_gaji,
			case 
				...
				end as nama,m_lokasi.kode as nm_lokasi,b.nominal,prl_gaji_detail_id,b.p_karyawan_id,a.m_lokasi_id
			from prl_gaji a 
			...
			join prl_generate_karyawan k on k.p_karyawan_id = b.p_karyawan_id and k.prl_generate_id = a.prl_generate_id and k.prl_generate_id = $id_prl $where_periode_gaji
			...
			where a.prl_generate_id = $id_prl
			$where_prl
			$where_periode_p_karyawan
			$where_periode_gaji
			and b.active=1
            ...
            foreach ($row as $row) {
                ...
                $data[$row->p_karyawan_id]['Keterangan'][$row->nama] = ($row->keterangan_gaji);
		...
		case when a.periode_gajian=0 then 'Pekanan' when a.periode_gajian=2 then 'Pekanan dan Bulanan' else 'Bulanan' end as tipe,
		a.tahun as tahun_gener,a.*,a.bulan as bulan_gener,b.tgl_awal as tgl_awal_lembur,b.tgl_akhir as tgl_akhir_lembur,periode_gajian
		FROM prl_generate a 
        ...
        $id_lokasi = Auth::user()->user_entitas;
        if ($id_lokasi and $id_lokasi != -1)
            $whereLokasigenerate = "and m_lokasi.m_lokasi_id  in($id_lokasi)";
        else {
            $whereLokasigenerate = "";
        }
        ...
        if (empty($list_karyawan) or !($request->get('preview') or $request->get('view') or $request->get('menu'))) {
            ...
            $sqluser = "SELECT prl_generate.*,prl_gaji.*
        ,case when prl_gaji.periode_gajian_in_gajian=0 then 'Pekanan' else 'Bulanan' end as tipe
        ,m_lokasi.nama as nmlokasi,
    	a.tgl_awal as tgl_awal_absen, 
    	a.tgl_akhir as tgl_akhir_absen, 
    	b.tgl_awal as tgl_awal_lembur, 
    	b.tgl_akhir as tgl_akhir_lembur,
    	a2.tgl_awal as tgl_awal_absen_2, 
    	a2.tgl_akhir as tgl_akhir_absen_2, 
    	b2.tgl_awal as tgl_awal_lembur_2, 
    	b2.tgl_akhir as tgl_akhir_lembur_2,
    	CASE  WHEN periode_gajian_in_gajian is null THEN prl_generate.periode_gajian ELSE periode_gajian_in_gajian END AS periode_gajian_in_gajian
,
    	prl_generate.tahun,
    	a.pekanan_ke,
            pgk.*
            ...
            left join (select prl_generate_id,lokasi_id,SUM(case when prl_generate_karyawan.on_off = 'ON' then 1 else 0 end) is_on, SUM(case when prl_generate_karyawan.on_off = 'OFF' then 1 else 0 end) is_off from prl_generate_karyawan group by prl_generate_id,lokasi_id
            ) pgk on pgk.prl_generate_id = prl_generate.prl_generate_id and pgk.lokasi_id = prl_gaji.m_lokasi_id
    		left join m_periode_absen a2 on 
	    		CASE 
	           	WHEN prl_gaji.periode_gajian_in_gajian= 1 THEN prl_generate.periode_absen_bulanan_id
	           	WHEN prl_gaji.periode_gajian_in_gajian= 0 THEN prl_generate.periode_absen_pekanan_id 
	         
	           END			= a2.periode_absen_id
    		left join m_periode_absen b2 on 
	    		CASE 
	           	WHEN prl_gaji.periode_gajian_in_gajian= 1 THEN prl_generate.periode_lembur_bulanan_id
	           	WHEN prl_gaji.periode_gajian_in_gajian= 0 THEN prl_generate.periode_lembur_pekanan_id
	         
	           END			= b2.periode_absen_id
        ...
        if ($request->get('Cari') == "RekapExcel") {
            ...
            return view('backend.gaji.preview.edit_karyawan', compact('data', 'data_row', 'data_head', 'entitas', 'user', 'id_prl', 'help', 'periode', 'periode_absen', 'request', 'list_karyawan', 'lokasi', 'bank', 'subpage', 'jabatan', 'sudah_appr', 'sudah_appr_keuangan', 'sudah_appr_hr', 'sudah_appr_voucher', 'generate_gaji'));
        } else if ($request->get('Cari') == "Absen") {
            $sqllokasi = "SELECT * FROM m_lokasi WHERE active=1 and sub_entitas=0 ORDER BY nama ASC ";
            $lokasi = DB::connection()->select($sqllokasi);
            ...
            $detail_absen = DB::connection()->select(
                "SELECT *,p_karyawan.nama as nama_lengkap 
		 	FROM prl_gaji_detail
    		 	join prl_gaji on prl_gaji_detail.prl_gaji_id = prl_gaji.prl_gaji_id
    		 	join p_karyawan on prl_gaji_detail.p_karyawan_id = p_karyawan.p_karyawan_id
    		 	join prl_generate_karyawan k on k.p_karyawan_id = p_karyawan.p_karyawan_id and k.prl_generate_id = prl_gaji.prl_generate_id
		 	where type=1 and gaji_absen_id=35 and prl_gaji.prl_generate_id=$id_prl
		 	$whereLokasirole
            ...
            return view('backend.gaji.preview.detail_absen', compact('data', 'detail_absen', 'data_row', 'data_head', 'entitas', 'user', 'id_prl', 'help', 'periode', 'periode_absen', 'request', 'list_karyawan', 'lokasi', 'bank', 'subpage', 'jabatan', 'sudah_appr', 'sudah_appr_keuangan', 'sudah_appr_hr', 'sudah_appr_voucher', 'generate_gaji'));
            ...
            return view('backend.gaji.preview.approval_hr', compact('data', 'data_row', 'data_head', 'entitas', 'user', 'id_prl', 'help', 'periode', 'periode_absen', 'request', 'list_karyawan', 'generate', 'appr', 'subpage', 'sudah_appr', 'sudah_appr_keuangan', 'sudah_appr_hr', 'generate_gaji'));
            ...
            $appr = DB::connection()->select($sql);
            return view('backend.gaji.preview.approval', compact('data', 'data_row', 'data_head', 'entitas', 'user', 'id_prl', 'help', 'periode', 'periode_absen', 'request', 'list_karyawan', 'generate', 'appr', 'subpage', 'sudah_appr', 'sudah_appr_keuangan', 'sudah_appr_hr', 'generate_gaji'));
            ...
            return view('backend.gaji.preview.approval_keuangan', compact('data', 'data_row', 'data_head', 'entitas', 'user', 'id_prl', 'help', 'periode', 'periode_absen', 'request', 'list_karyawan', 'generate', 'subpage', 'appr', 'sudah_appr', 'sudah_appr_keuangan', 'sudah_appr_hr', 'generate_gaji'));
            ...
            return view('backend.gaji.preview.ajuan', compact('data', 'data_row', 'data_head', 'entitas', 'user', 'id_prl', 'help', 'periode', 'periode_absen', 'request', 'list_karyawan', 'generate', 'sudahappr', 'subpage', 'sudah_appr', 'sudah_appr_voucher', 'generate_gaji'));
            ...
            if ($request->get('menu') == 'Presensi') {
                ...
                return view('backend.gaji.preview.view_presensi' . $pekanan, compact('data', 'data_row', 'data_head', 'entitas', 'user', 'id_prl', 'help', 'periode', 'periode_absen', 'request', 'list_karyawan', 'subpage', 'sudah_appr', 'sudah_appr_keuangan', 'sudah_appr_hr', 'sudah_appr_voucher', 'generate_gaji'));
            } else if ($request->get('menu') == 'previewpajak') {
                return view('backend.gaji.preview.view_previewpajak' . $pekanan, compact('data', 'data_row', 'data_head', 'entitas', 'user', 'id_prl', 'help', 'periode', 'periode_absen', 'request', 'list_karyawan', 'subpage', 'sudah_appr', 'sudah_appr_keuangan', 'sudah_appr_hr', 'sudah_appr_voucher', 'generate_gaji'));
            } else {

                return view('backend.gaji.preview.viewgapreview', compact('data', 'data_row', 'data_head', 'entitas', 'user', 'id_prl', 'help', 'periode', 'periode_absen', 'request', 'list_karyawan', 'generate', 'subpage', 'sudah_appr', 'sudah_appr_keuangan', 'sudah_appr_hr', 'sudah_appr_voucher', 'generate_gaji'));
            }
        }
    }
    ...
    public function undo_appr_gaji($request)
    {
        DB::beginTransaction();
        try {
            DB::enableQueryLog();
            ...
            for ($i = 0; $i < count($list_pajak); $i++) {
                ...
                DB::connection()->table("prl_gaji")
                    ...
                    ->where('m_lokasi_id', $entitas)
                    ->update([
                        "appr_" . $pajak . "_direktur_status" => (3),
                        "appr_" . $pajak . "_direktur_by" => $idUser,
                        "appr_" . $pajak . "_direktur_date" => date('Y-m-d H:i:s'),
    ...
    public function simpan_konfirm_gaji($request)
    {
        ...
        try {
            ...
            for ($i = 0; $i < count($list_pajak); $i++) {
                ...
                $ipg = DB::connection()->select("select * from prl_gaji b 
	           join prl_gaji_detail a on a.prl_gaji_id = b.prl_gaji_id 
	            where prl_generate_id = $id and b.m_lokasi_id = $entitas and gaji_absen_id=32 and keterangan is not null");
                foreach ($ipg as $ipg) {
                    $tgl = explode('|', $ipg->keterangan);
                    for ($i = 0; $i < count($tgl); $i++) {
                        if (!empty($tgl[$i])) {

                            DB::connection()->table("m_hari_libur_cuti_ipg")
                                ->insert([

                                    "p_karyawan_id" => ($ipg->p_karyawan_id),
                                    "tanggal" => ($tgl[$i]),

                                    "prl_gaji_id" => ($ipg->prl_gaji_id),
                                    "prl_generate_id" => ($ipg->prl_generate_id),
                                    "prl_gaji_detail_id" => ($ipg->prl_gaji_detail_id),
                                    "active" => (1),
                                    "create_by" => $idUser,
                                    "create_date" => date("Y-m-d H:i:s"),
                                ]);
    ...
    public function save_keterangan_edit_ajuan(Request $request)
    {
        ...
        try {
            ...
            if ($request->update_master) {
                DB::connection()->table("p_karyawan_pekerjaan")
                    ->where('p_karyawan_id', $request->id_karyawan)
                    ->update([
                        "m_bank_id" => $request->bank,
                        "pajak_onoff" => $request->pajakonoff,
                        "norek" => $request->norek,
                    ]);
        ...
        try {
            ...
            if ($id_nominal != -1) {
                ...
                if ($field == 'KKB') {
                    ...
                    $id = 17;
                    $type = 3;
                } else if ($field == 'lemburproposional') {
                    $id = 21;
                    $type = 3;
    ...
    public function exportsGaji($param)
    {
        ...
        $list_karyawan = $param['list_karyawan'];
        $hisentitas = $param['hisentitas'];
        $generate_gaji = $param['generate_gaji'];
        ...
        if (!empty($list_karyawan)) {
            ...
            foreach ($list_karyawan as $list_karyawan) {
                ...
                $sheet->setCellValue($help->toAlpha($i) . $rows, $masa);
                $i++;
                $sheet->setCellValue($help->toAlpha($i) . $rows, $list_karyawan->nmlokasi);
                $i++;
                $sheet->setCellValue($help->toAlpha($i) . $rows, $list_karyawan->pajak_onoff);
