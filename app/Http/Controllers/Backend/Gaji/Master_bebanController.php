...
class Master_bebanController extends Controller
{
    /**
    ...
    public function master_beban (Request $request)
    {
		...
		$title='master beban';
		$sql="SELECT * FROM m_beban"; 
		$row=DB::connection()->select($sql);
        return view('backend.gaji.master.beban.beban',compact('user','row','title'));
    } public function tambah(){
		$data['nama']='';
		
		$data['active']='';
		$data['persentase']='';
		$data['is_beban_karyawan']='';
		$id='';
		$title='master beban';
		$type='simpan_'.str_replace(' ','_',$title);
		$page='Tambah';
		return view('backend.gaji.master.beban.tambah_edit_beban',compact('data','type','id','page','title','title'));
	}public function edit($id){
		$page='Edit';
		$title='master beban';
		$type='update_'.str_replace(' ','_',$title);
		$sql = "SELECT * FROM m_beban where m_beban_id=$id ";
    	$cos=DB::connection()->select($sql);
		$data['nama']=$cos[0]->nama;
		$data['persentase']=$cos[0]->persentase;
		$data['is_beban_karyawan']=$cos[0]->is_beban_karyawan;
		
		$data['active']=$cos[0]->active;
		
		return view('backend.gaji.master.beban.tambah_edit_beban',compact('data','type','id','page','title'));
        ...
        try{
            ...
            DB::connection()->table("m_beban")
                ->insert([
                    ...
                    "is_beban_karyawan"=>($request->get("is_beban_karyawan")),
                    "active"=>(1),
