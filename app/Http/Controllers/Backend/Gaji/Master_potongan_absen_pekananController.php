...
class Master_potongan_absen_pekananController extends Controller
{
    /**
    ...
    public function master_potongan_absen (Request $request)
    {
        ...
        return view('backend.gaji.master.potongan_absen.potongan_absen_pekanan	',compact('user','row','title'));
        ...
        try{ 
            $idUser=Auth::user()->id;
             DB::connection()->table("m_potongan_absen_pekanan")
		                ->delete();
            $sql = "SELECT * FROM m_pangkat where active=1";
    			$pangkat=DB::connection()->select($sql);
            	foreach($pangkat as $pangkat){
		            DB::connection()->table("m_potongan_absen_pekanan")
		                ->insert([
		                    "type_nominal"=>($request->get("type_absen")[$pangkat->m_pangkat_id]),
		                    "nominal"=>($request->get("absen")[$pangkat->m_pangkat_id]),
		                    "m_pangkat_id"=>($pangkat->m_pangkat_id),
		                    "type_absen"=>'absen',
		                   
		                ]);
		                DB::connection()->table("m_potongan_absen_pekanan")
		                ->insert([
		                    "type_nominal"=>($request->get("type_fingerprint")[$pangkat->m_pangkat_id]),
		                    "nominal"=>($request->get("fingerprint")[$pangkat->m_pangkat_id]),
		                    "m_pangkat_id"=>($pangkat->m_pangkat_id),
		                    "type_absen"=>'fingerprint',
		                   
		                ]);DB::connection()->table("m_potongan_absen_pekanan")
		                ->insert([
		                    "type_nominal"=>($request->get("type_pm")[$pangkat->m_pangkat_id]),
		                    "nominal"=>($request->get("pm")[$pangkat->m_pangkat_id]),
		                    "m_pangkat_id"=>($pangkat->m_pangkat_id),
		                    "type_absen"=>'pm',
