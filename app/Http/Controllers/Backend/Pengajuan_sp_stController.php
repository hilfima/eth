...
class Pengajuan_sp_stController extends Controller
{
    ...
    }public function pengajuan_sp_st(Request $request)
	{
		...
		$sqlfasilitas="SELECT *,p_karyawan.nama,aju.nama as pengaju FROM p_karyawan_sanksi
		left join p_karyawan aju on p_karyawan_id_yang_mengajukan = aju.p_karyawan_id
		...
		return view('backend.pengajuan_sp_st.pengajuan_sp_st',compact('pengajuan_sp_st'));
	}
	public function edit_pengajuan_sp_st($id)
	{
		...
		$data['p_karyawan_id_yang_mengajukan']=$sanksi[0]->p_karyawan_id_yang_mengajukan;
		$data['status']=$sanksi[0]->status;
		...
		return view('backend.pengajuan_sp_st.tambah_pengajuan_sp_st',compact('id','data','type','karyawan','jenis_sanksi'));
	}
	...
	}public function simpan_pengajuan_sp_st(Request $request)
	{
		...
		return redirect()->route('be.pengajuan_sp_st')->with('success',' Pengajuan Berhasil di input!');
	}public function update_pengajuan_sp_st (Request $request, $id)
	{
		...
		return redirect()->route('be.pengajuan_sp_st')->with('success',' kotak_laporan Berhasil di input!');
