...
where users.id=$iduser";
    ...
    public function optimasi_today_info(){
        ...
        $pjs = DB::connection()->select("select *,p_karyawan.nama as nama,m_jenis_ijin.nama as nama_ijin from t_permit 
        join p_karyawan on t_permit.p_karyawan_id = p_karyawan.p_karyawan_id 
        join m_jenis_ijin on t_permit.m_jenis_ijin_id = m_jenis_ijin.m_jenis_ijin_id 
        where pjs = $id and to_char(tgl_awal,'YYYY-MM-DD') >='" . date('Y-m-d') . "' and '" . date('Y-m-d') . "' <=to_char(tgl_akhir,'YYYY-MM-DD') and ((status_appr_1 = 1 and t_permit.m_jenis_ijin_id not in (22)) 
						or (status_appr_2 = 1 and t_permit.m_jenis_ijin_id in (22)) ) ");
						
    	if(!empty($pjs)):             
        	foreach($pjs as $pjs):
                $today[] = array('Anda Menjadi Penanggung Jawab Sementara dari Perkerjaan '. $pjs->nama.' <br>'.$pjs->nama.' tidak masuk hari ini karena '.$pjs->nama_ijin,'javascript:void(0)');
            endforeach;        
         endif;        
...
foreach($berita as $ber){
	$today[] = array(str_ireplace(array('Powered By','Froala Editor','href="https://www.froala.com/wysiwyg-editor'),array('','','href="#"'),$ber->deskripsi),'');
}
    ...
    ', route('fe.lihat_agenda_perusahaan', $agenda->t_agenda_perusahaan_id));
    				...
    				join p_karyawan_kontrak on p_karyawan_kontrak.p_karyawan_id = p_karyawan.p_karyawan_id and p_karyawan_kontrak.active=1
    					...
    					";
    						...
    						where  ((status_appr_1 = 3 and t_permit.m_jenis_ijin_id not in (22) and appr_1=$id_karyawan) 
    						or (status_appr_2 = 3 and  appr_2=$id_karyawan and t_permit.m_jenis_ijin_id in (22)) ) and tgl_awal>='$tgl_awal_gaji' and tgl_awal<='$tgl_akhir_gaji'
    ...
    public function optimasi_berita(){    
    	$help = new Helper_function();
        $iduser = Auth::user()->id;
        $sqlidkar = "select *,p_karyawan.p_karyawan_id as karyawan_id from p_karyawan 
        			left join p_karyawan_pekerjaan a on a.p_karyawan_id = p_karyawan.p_karyawan_id 
        ...
        foreach($notifikasi_berita as $berita){
        	$content .=$berita->notif."
        	<hr>
        	";
        }
    	$return['content'] = $content;
        $notifikasi_berita = DB::connection()->table("notifikasi")->where('p_karyawan_id',$id)->update(["terbaca"=>1,"date_terbaca"=>date('Y-m-d')]);
    	echo json_encode($return);
