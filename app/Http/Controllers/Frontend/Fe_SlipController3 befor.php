...
class Fe_SlipController extends Controller
{
    ...
    { 
        ...
        if( $request->get('bulan') and $request->get('tahun')){
         ...
         $gaji=DB::connection()->select("select * from prl_generate 
        join m_periode_absen on prl_generate.periode_absen_id = m_periode_absen.periode_absen_id
        join prl_gaji on prl_gaji.prl_generate_id = prl_generate.prl_generate_id and m_lokasi_id = ".$id_lokasi." 
        
        where prl_generate.tahun = ".$request->get('tahun')." and prl_generate.bulan = ".$request->get('bulan')." and $r=1  and prl_generate.periode_gajian = ".$request->get('periode_gajian')." and prl_generate.active=1  $where_pekanan ");
        	if(count($gaji))
...
WHERE a.p_karyawan_id=$id";
		...
		if($request->get('Cari')=='PDF'){
           ...
           'row'=> $row];
			 ...
			 $pdf = PDF::loadView('frontend.slip.pdf_slip', $data);
			 
			    $dompdf = $pdf->getDomPDF();
				$canvas = $dompdf->get_canvas();
				
				$dompdf->render();
			return	$dompdf->stream('Slip Gaji.pdf', array("Attachment" => false));
			// return $pdf->download('Slip Gaji.pdf');
       
		}else{
       return view('frontend.slip.list_slip_2',compact('data','help','periode','periode_absen','id_prl','id_kary','help','periode_absen','karyawan','generate','row','idkar','request')); 
       }
