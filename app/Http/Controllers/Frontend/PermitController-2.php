...
*/
	...
	public function listed(Request $request)
	{
		...
		$tgl_akhir_gaji = $gapen[0]->tgl_awal;
	...
	public function tambah_cuti()
	{
		...
		return view('frontend.permit.tambah_cuti',compact('kar','jeniscuti','appr','help','karyawan','totalcuti','tgl_cut_off','idkar'));
	}
...
$tolcut = $cuti[0]->sisa_cuti;
			...
			if(!count($gapen)){
				...
				return view('frontend.permit.tambah_izin',compact('kar','jenisizin','appr','tolcut','karyawan','totalcuti','tgl_cut_off','idkar'));
		}
	}
	public function tambah_perdin()
	{
		...
		$sql="select * from m_periode_absen where type= ".$idkar[0]->periode_gajian." and periode_aktif=1  and tgl_akhir >= '".date('Y-m-d')."'  and tgl_akhir >= '".date('Y-m-d')."' ";
			...
			if(!count($gapen)){
				...
				return view('frontend.permit.tambah_perdin',compact('kar','jenisizin','appr','karyawan','tgl_cut_off','idkar'));
		}
	}
	...
	public function tambah_lembur()
	{
		...
		$sqlappr="SELECT * from get_data_karyawan() WHERE m_jabatan_id in($atasan) and m_pangkat_id in(5,6) ";
		...
		return view('frontend.permit.tambah_lembur',compact('kar','jenisizin','appr1','appr2','karyawan','tgl_cut_off','idkar'));
		}
	}
